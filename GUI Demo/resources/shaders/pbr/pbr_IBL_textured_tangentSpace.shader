...
void main()
{
	...
	// with reorthogonalizing - no need for the bitangent!
...
uniform sampler2D depthMap; // parallax mapping

uniform float height_scale; // scale for parallax mapping
...
// Fresnel equation for IBL lighting https://seblagarde.wordpress.com/2011/08/17/hello-world/
...
{
	float numLayers = mix(maxLayers, minLayers, min(0.0, max(dot(vec3(0.0, 0.0, 1.0), viewDir), 0.0)));
...
void main()
{
	vec3 V = TBN_transpose * normalize(camPos - WorldPos); // Normal vector pointing from WorldPos of fragment -> camPos

	// Offset texture coordinates based on parallax mapping
	vec2 TexCoordsNew = ParallaxMapping(TexCoords, V);
	// Use the new texcoords for further sampling!
	//if (TexCoordsNew.x > 1.0 || TexCoordsNew.y > 1.0 || TexCoordsNew.x < 0.0 || TexCoordsNew.y < 0.0) // clip borders
		//discard;

	vec3 albedo = pow(texture(albedoMap, TexCoordsNew).rgb, vec3(2.2)); // TODO: Is this correct?
	// Sample normal in tangent space
	vec3 normal = texture(normalMap, TexCoordsNew).rgb;
	normal = normal * 2.0 - 1.0;
	normal = normalize(normal);
	float metallic = texture(metallicMap, TexCoordsNew).r;
	float roughness = texture(roughnessMap, TexCoordsNew).r;
	float ao = texture(aoMap, TexCoordsNew).r;
	...
	for (int i = 0; i < 1; ++i)
	{
		vec3 L = TBN_transpose * normalize(lightPositions[i] - WorldPos); // Normal vector pointing from WorldPos -> lightPositions[i]
	...
	vec3 irradiance = texture(irradianceMap, TBN * N).rgb;
	vec3 diffuse = irradiance * albedo;

	// Specular IBL
	vec3 R = reflect(-V, N);
	const float MAX_REFLECTION_LOD = 4.0;
	vec3 prefilteredColor = textureLod(prefilterMap, TBN * R, roughness * MAX_REFLECTION_LOD).rgb;

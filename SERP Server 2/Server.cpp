...
void Server::sendMessageResponse(const SnackerEngine::SERPRequest& request, Client& client, SnackerEngine::ResponseStatusCode responseStatusCode, const std::string& message)
{
...
bool Server::prepareForRelay(const SnackerEngine::SERPMessage& message, Client& source)
{
	...
	if (message.getHeader().source != source.serpID) {
		if (message.isRequest()) {
			sendMessageResponse(static_cast<const SnackerEngine::SERPRequest&>(message), source, SnackerEngine::ResponseStatusCode::BAD_REQUEST, "Attempted to relay message but gave incorrect serpID as source!");
...
void Server::relayRequest(Client& source, SnackerEngine::SERPID destination, std::unique_ptr<SnackerEngine::SERPMessage> request)
{
	...
	if (destinationClient) {
		printMessage("[DEBUG]: source: " + SnackerEngine::to_string(request->getHeader().source) + ", destination: " + SnackerEngine::to_string(request->getHeader().destination) + ", messageID: " + SnackerEngine::to_string(request->getHeader().messageID));
		...
		sendMessageResponse(static_cast<const SnackerEngine::SERPRequest&>(*request), source, SnackerEngine::ResponseStatusCode::NOT_FOUND, "no client with serpID " + SnackerEngine::to_string(destination) + " is currently connected.");
...
void Server::relayResponse(Client& source, SnackerEngine::SERPID destination, std::unique_ptr<SnackerEngine::SERPMessage> response)
{
	...
	std::shared_ptr<Client> destinationClient = getClient(destination);
	if (destinationClient) {
		printMessage ("[DEBUG]: source: " + SnackerEngine::to_string(response->getHeader().source) + ", destination: " + SnackerEngine::to_string(response->getHeader().destination) + ", messageID: " + SnackerEngine::to_string(response->getHeader().messageID));
...
void Server::handleIncomingMessage(Client& client)
{
	...
	printMessage("[DEBUG] Received " + SnackerEngine::to_string(result.size()) + " messages.");
	for (unsigned int i = 0; i < result.size(); ++i) {
		printMessage("[DEBUG] messagedID of message " + SnackerEngine::to_string(i+1) + ":" + SnackerEngine::to_string(result[i]->getHeader().messageID));
...
void Server::run()
{
	...
	while (true) {
		...
		{
			std::lock_guard lock(clientsMapMutex);
			numberOfConnectedClients = clients.size();
		}
